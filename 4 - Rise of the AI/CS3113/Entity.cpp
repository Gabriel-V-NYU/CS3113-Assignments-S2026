...
void Entity::init(Vector2 position, Vector2 scale, const char *textureFilepath,
...
void Entity::AIWander(Map *map)
{
    ...
    float forwardX = mPosition.x + (mDirection == RIGHT ? 1.0f : -1.0f) * (mColliderDimensions.x / 2.0f + 1.0f);

    bool wallAhead = map && map->isSolidTileAt({ forwardX, mPosition.y }, &xOverlap, &yOverlap);

    bool edgeAhead = map && !map->isSolidTileAt({ forwardX, mPosition.y + mColliderDimensions.y / 2.0f + 1.0f }, &xOverlap, &yOverlap);
...
void Entity::AIFollow(Entity *target)
{
    ...
    {
    case IDLE:
        if (Vector2Distance(mPosition, target->getPosition()) < 50.0f) 
            mAIState = WALKING;
        break;
...
void Entity::AILerp(Entity *target)
{
    if (target == nullptr) return;

    if (Vector2Distance(mPosition, target->getPosition()) < 50.0f) return;

    float lerpSpeed = 3.0f;
    mPosition = Vector2Lerp(mPosition, target->getPosition(), lerpSpeed * GetFrameTime());
    mVelocity = { 0.0f, 0.0f };
